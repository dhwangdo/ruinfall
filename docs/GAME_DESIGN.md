...
- 플레이어 주변 3×3만 밝혀진다.
- 아직 밝혀지지 않은 칸은 그리드 자체를 그리지 않는다.
- 카메라는 자유롭게 드래그할 수 있고, 휠 확대/축소에는 최솟값과 최댓값이 있다.
- 이미 방문하고 전투가 정리된 방들로 경로가 연결되면 먼 방도 클릭할 수 있다.
- 여러 칸 이동 시간은 이동 칸 수의 제곱근에 대략 비례한다.
...
`?`는 들어갈 때 전투 1/3, 빈 방 2/3으로 결정된다.

각 지역의 마지막 줄에서는 포탈 확률 10%가 추가된다. 그 줄의 `?` 비율은 58%가 되며, 포탈이 하나도 생성되지 않으면 진행을 위해 하나를 보장한다.
...
- 덱 케이스 최대 보유 수: 3
- 시작 덱 케이스 용량: 20
- 시작 카드: 타격 5장, 방어 5장, 무작위 특별 카드 5장
